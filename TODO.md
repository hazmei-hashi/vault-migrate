...
- 289 tests passing across 7 packages (E2E gated behind `E2E_TESTS=1`, 6
  scenarios verified against a real Vault 1.18.5 cluster this session,
  not counted in the 289)
- Coverage: `client` 52.6%, `cmd` 34.5%, `config` 100.0%, `kvv2` 82.9%, `state` 85.5%; **total 77.8%**
...
- [x] Add coverage gate (fail build if coverage drops below target) — 73% floor, measured 77.8%
